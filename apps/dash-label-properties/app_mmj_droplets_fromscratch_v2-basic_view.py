...
def update_output(num_clicks, val_selected):
    if val_selected is None:
        ...
        try:
            ...
            fig = cout_data_figure(img, props_table, props_table, label_array, active_columns=['label', 'area'], color_column="area")
            # https://community.plotly.com/t/updating-a-dropdown-menus-contents-dynamically/4920/5
            drop_menu = dcc.Dropdown(id="color-drop-menu",
                                     options=[{"label": col_name.capitalize(), "value": col_name}
                                              for col_name in props_table.columns],
                                     value="label", )
            print(' ... Data ready for output.')
