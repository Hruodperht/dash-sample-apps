...
def image_with_contour(img, active_labels, data_table, active_columns, color_column):
    ...
    fig.update_layout(margin=dict(l=0, r=0, b=0, t=0, pad=0), template="plotly_dark")  # "simple_white"
        ...
        [
            dbc.Row(
                [
                    ...
                    dbc.Col(dbc.NavbarBrand("MMJ Drop Segmentation App")),
                    # modal_overlay,
                ],
                align="center",
            ),
...
file_card = dbc.Card([
        ...
        dbc.CardBody(
            [
                ...
                dbc.Col(
                    html.Div(id='output-data-upload'), align="left", md=6
                )
...
def highlight_filter(
    indices, cell_index, data, active_columns, color_column,
        list_of_contents, previous_row, list_of_names, list_of_dates):
    """
    ...
    fig = image_with_contour(
        img_in, filtered_labels, filtered_table, active_columns, color_column
    )

    if cell_index and cell_index["row"] != previous_row:
        label = filtered_labels[cell_index["row"]]
        mask = (label_array_in == label).astype(np.float)
        contour = measure.find_contours(label_array_in == label, 0.5)[0]
        # We need to move the contour left and up by one, because
        # we padded the label array
        y, x = contour.T - 1
        ...
        return [fig, cell_index["row"], False, children]

    return [fig, previous_row, False, children]
