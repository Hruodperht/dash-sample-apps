...
def create_dash_data_table(table, initial_columns):
...
def get_row_names(chosen_file_path_from_dropdown, options):
    ...
    if chosen_file_path_from_dropdown is None:
        dt = create_dash_data_table(pd.DataFrame(), ["label"])
        dt = None
        ...
        try:
            ...
            dt = create_dash_data_table(probs_table.copy(deep=True), initial_columns)
            ...
            fig = create_plotly_figure(img,
                                       ...
                                       initial_columns,
                                       colorbar_label)
            graph_children = [dcc.Graph(id="actual-graph", figure=fig)]
...
def higlight_row(string, chosen_file_path_from_dropdown):
    """
