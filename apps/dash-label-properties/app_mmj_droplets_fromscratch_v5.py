...
def update_output(num_clicks, drop_menu_state_options, drop_menu_state_selected, inserted_path):
    if inserted_path is None or inserted_path == "":
        raise PreventUpdate
    if len(drop_menu_state_options) == 0 and (inserted_path is None or inserted_path == ""):
        raise PreventUpdate
    elif drop_menu_state_selected is not None:
        try:
